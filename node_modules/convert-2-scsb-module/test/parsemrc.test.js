...
describe('mapUtils lib/utils.js', function () {
  ...
  it('It should parse a marc file record and set the size, binary, and js objs', function (done) {
    H(new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_too_large_record.mrc')))
      .map((record) => {
  ...
  it('It should extract the bnumber', function (done) {
    H(new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_has_bnumber.mrc')))
      .map((record) => {
        record = parsLib.convertToJsonCheckSize(record)
  ...
  it('It should extract the OCLC number', function (done) {
    H(new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_has_oclc.mrc')))
      .map((record) => {
        record = parsLib.convertToJsonCheckSize(record)
  ...
  it('It should extract the Bib level call number', function (done) {
    H(new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_has_bib_call_number.mrc')))
      .map((record) => {
        record = parsLib.convertToJsonCheckSize(record)
  ...
  it('It should extract control fields', function (done) {
    H(new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_multiple_items.mrc')))
      .map((record) => {
        record = parsLib.convertToJsonCheckSize(record)
  ...
  it('It should extract the 866 holdings data', function (done) {
    H(new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_multiple_866_1.mrc')))
      .map((record) => {
        record = parsLib.convertToJsonCheckSize(record)
  ...
  it('It should fix the 001 when there is no OCLC', function (done) {
    H(new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_no_001_no_991.mrc')))
      .map((record) => {
        record = parsLib.convertToJsonCheckSize(record)
        record.bNumber = parsLib.extractBnumber(record.mij)
        record.oclcNumber = parsLib.extractOclc(record.mij)
  ...
  it('It should build the items object', function (done) {
    H(new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_multiple_items.mrc')))
      .map((record) => {
        parsLib.registerLogger('test/marcs/test_tmp.mrc')
        record = parsLib.convertToJsonCheckSize(record)
        record.bNumber = parsLib.extractBnumber(record.mij)
        record.oclcNumber = parsLib.extractOclc(record.mij)
        record.bibCallNumber = parsLib.extractBibCallNumber(record.mij)
  ...
  it('It should build the items object multiple items one 866', function (done) {
    H(new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_multiple_callnumbers.mrc')))
      .map((record) => {
        parsLib.registerLogger('test/marcs/test_tmp.mrc')
        record = parsLib.convertToJsonCheckSize(record)
        record.bNumber = parsLib.extractBnumber(record.mij)
  ...
  it('It should build the holdings 852 and 866 statements - multiple call numbers no 866', function (done) {
    H(new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_multiple_callnumbers_no_866.mrc')))
      .map((record) => {
        parsLib.registerLogger('test/marcs/test_tmp.mrc')
        record = parsLib.convertToJsonCheckSize(record)
        record.bNumber = parsLib.extractBnumber(record.mij)
  ...
  it('Use restriction generation - In Library Use 2', function () {
    var data852 = {
      3: 'v. 1  1956',
      b: 'rcma2',
      h: '*QX (Mětšk, F. Chrestomatija dolnoserbskego pismowstwa)'
  ...
  it('Rule check - Oct 2016 - Rule 1', function () {
    var data852 = { '3': false, b: 'rcma2', h: false }
    var data876 = { o: '2',
      s: '217',
      y: '55',
  ...
  it('Rule check - Oct 2016 - Rule 2', function () {
    var data852 = { '3': false, b: 'rcma2', h: false }
    var data876 = { o: 'u',
      s: '212',
      y: '55',
  ...
  it('Rule check - Oct 2016 - Rule 3', function () {
    var data852 = { '3': false, b: 'rcma2', h: false }
    var data876 = { o: '2',
      s: '227',
      y: '65',
    ...
    var r = parsLib.buildUseRestriction(data852, data876, 'NH')
    r.useRestriction.should.equal('In Library Use')
    r.groupDesignation.should.equal('Shared')
  })
  it('Rule check - Oct 2016 - Rule 4', function () {
    var data852 = { '3': false, b: 'rcma2', h: false }
    var data876 = { o: 'a',
      s: '212',
      y: '66',
    ...
    var r = parsLib.buildUseRestriction(data852, data876, 'NB')
    r.useRestriction.should.equal('In Library Use')
    r.groupDesignation.should.equal('Private')
  })
  it('Rule check - Oct 2016 - Rule 5', function () {
    var data852 = { '3': false, b: 'rcma2', h: false }
    var data876 = { o: '2',
      s: '213',
      y: '65',
    ...
    var r = parsLib.buildUseRestriction(data852, data876, 'JO')
    r.useRestriction.should.equal('In Library Use')
    r.groupDesignation.should.equal('Private')
  })
  ...
  it('Export all examples', function (done) {
    ...
    var allTests = [
      new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_a_lot_of_items.mrc')),
      new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_has_bib_call_number.mrc')),
      new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_has_bnumber.mrc')),
      new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_has_non_recap_items.mrc')),
      new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_has_oclc.mrc')),
      new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_multiple_866_1.mrc')),
      new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_multiple_866_2.mrc')),
      new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_multiple_callnumbers_no_866.mrc')),
      new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_multiple_callnumbers.mrc')),
      new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_multiple_items.mrc')),
      new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_no_001_has_991.mrc')),
      new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_no_001_no_991.mrc')),
      new marc.Iso2709Reader(fs.createReadStream('test/marcs/test_too_large_record.mrc'))
    ]
